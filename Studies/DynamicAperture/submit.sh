...
line=sps_q20_inj.json
initial_conditions=initial_conditions_uniform_ring_6d.json
python_script=script.py

cp $SUBMISSION_DIR/$line .
cp $SUBMISSION_DIR/$initial_conditions .
cp $SUBMISSION_DIR/$python_script .

tar -czf files.tar.gz $line $initial_conditions $python_script xobjects xcoll xpart xtrack xdeps xfields
